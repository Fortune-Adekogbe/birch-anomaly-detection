...
def execute_query(query_sql: str) -> pd.DataFrame:
    ...
    try:
        ...
        skab_df = pd.DataFrame(data=json_response['dataset'], columns=[
                               i['name'] for i in json_response['columns']])
...
def process_data(df: pd.DataFrame) -> tuple:
    '''
    This function cleans and processes an input dataframe and returns a tuple containing the 
    cleaned dataframe and the target column.
    '''
    del df['changepoint']
...
# create dataframe
skab_df = execute_query(
    "SELECT * FROM alldata_skab.csv WHERE anomaly IN('0.0', '1.0');")
